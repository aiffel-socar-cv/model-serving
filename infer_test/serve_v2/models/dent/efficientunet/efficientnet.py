...
class EfficientNet(nn.Module):
    def __init__(self, block_args_list, global_params):
        ...
        self._conv_stem = Conv2dSamePadding(
            in_channels, out_channels, kernel_size=3, stride=2, bias=False, name="stem_conv"
        )
        self._bn0 = BatchNorm2d(
            num_features=out_channels, momentum=batch_norm_momentum, eps=batch_norm_epsilon, name="stem_batch_norm"
        )
        ...
        self._bn1 = BatchNorm2d(
            num_features=out_channels, momentum=batch_norm_momentum, eps=batch_norm_epsilon, name="head_batch_norm"
        )
    ...
    try:
        if pretrained:
            if len(model_name) < 16:
                ...
                pretrained_state_dict = torch.load(IMAGENET_WEIGHTS[model_name], map_location=torch.device("cpu"))
